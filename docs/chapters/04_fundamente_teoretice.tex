...
combinație de accelerometre, giroscoape și, opțional, magnetometre. Aceste componente sunt realizate tehnologic sub formă de sisteme micro-electro-mecanice (MEMS), 
care permit integrarea pe cipuri de dimensiuni mici, 
cu un consum energetic redus, ideal pentru dispozitivele purtabile \cite{woodman_2007}.

\subsection{Accelerometrul: Principiul de funcționare și ecuații}
\hspace{0.75cm}Accelerometrul MEMS măsoară accelerația liniară a dispozitivului relativ la un sistem de referință inerțial. 
...
\hspace{0.75cm}Este fundamental de înțeles că un accelerometru nu măsoară accelerația coordonată (derivata a doua a poziției), ci \textit{accelerația proprie} (proper acceleration). 
...
Deviația bias-ului în timp (drift) reprezintă o problemă majoră în sistemele de navigație inerțială (INS), deoarece integrarea dublă a erorii 
pentru obținerea poziției duce la o creștere pătratică a erorii de poziție în timp ($error \propto t^2$) \cite{titterton_2004}. 
...
\subsection{Giroscopul: Măsurarea vitezei unghiulare}
...
\subsection{Fuziunea Senzorială (Sensor Fusion)}
...
\subsection{Fotopletismografia (PPG): Principiu de funcționare}
...
Dacă scorul este scăzut sub baseline-ul personal al pacientului, aplicația va recomanda automat reducerea intensității exercițiilor pentru acea zi, prevenind riscul de recidivă.
